<form class="pt-2 relative mx-auto text-gray-600" autocomplete="off">
    ...
    @if ($search>0)
        ...
        <ul class="absolute bg-white w-full mt-1 rounded-lg overflow-hidden">
            @forelse ($this->results as $result)
                <li class="px-5 leading-10 text-sm cursor-pointer hover:bg-gray-300">
                    <a href="{{ route('courses.show', $result) }}">{{ $result->title }}</a>
                </li>

            @empty
            <li class="px-5 leading-10 text-sm">
